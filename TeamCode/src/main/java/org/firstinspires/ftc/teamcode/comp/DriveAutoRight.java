...
@Autonomous(name="Drive Auto Right", group="comp")
//@Disabled
public class DriveAutoRight extends LinearOpMode {
    ...
    public void runOpMode()  {
        ...
        while (opModeIsActive() && tank.isBusy() && runtime.seconds() < timeout) {
            telemetry.addData("Landing Run Forwards: ", "%2.5f S Elapsed", runtime.seconds());
            telemetry.update();
        }
        tank.stop();
        tank.resetEncoder();
        ...
        */
        ...
        switch (goldLocation) {
            ...
            case "CENTER": goldMove = 7.0; break;
            default: goldMove = -8.0; break;
        }
        ...
        runtime.reset();
        timeout = 0.5; //3.0;
        tank.imuTurn(imu, TURN_SPEED, TURN_SPEED / 3, runtime, timeout);

        // Driving to depot: drive forward for 30 inches ------------------------------------------
        runtime.reset();
        timeout = 4.0; //5.0;
        tank.encoderDrive(FORWARD_SPEED, 38.00 + goldMove, runtime, timeout, true);
        while (opModeIsActive() && tank.isBusy() && (runtime.seconds() < timeout)) {
            telemetry.addData("Path to depot 1", "Leg 1: %2.5f S Elapsed", runtime.seconds());
            telemetry.update();
        }
        ...
        runtime.reset();
        timeout = 6.0; //3.0;
        tank.imu2StepTurn(imu, TURN_SPEED, TURN_SPEED/4,-135, runtime, timeout);
        ...
        runtime.reset();
        timeout = 0.5; //3.0;
        tank.imuTurn(imu, TURN_SPEED/2, -135, runtime, timeout);
        ...
        //After dropping make sure at 45
        runtime.reset();
        tank.resetEncoder();
        tank.encoderMode(); //reset from runToPosition mode
        timeout = 1;
        tank.imuTurn(imu, TURN_SPEED/3, -135, runtime, timeout);
        while (opModeIsActive() && (runtime.seconds() < timeout)) {
            telemetry.addData("Landing Adjustment", "Leg 1: %2.5f S Elapsed", runtime.seconds());
            telemetry.update();
        }
