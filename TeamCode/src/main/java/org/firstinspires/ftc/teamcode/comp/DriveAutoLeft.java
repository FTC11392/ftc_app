...
public class DriveAutoLeft extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        */
        ...
        runtime.reset();
        timeout = 0.5; //3.0;
        tank.imuTurn(imu, TURN_SPEED/2, 45, runtime, timeout);

        // Driving to depot:  Drive Backwards for 48 inches ---------------------------------------
        runtime.reset();
        timeout = 2.0; // 6.0;
        tank.encoderDrive(FORWARD_SPEED, 37.00, runtime, timeout, true);
        while (opModeIsActive() && tank.isBusy() && runtime.seconds() < timeout) {
        ...
        tank.encoderMode(); //reset from runToPosition mode
        timeout = 1;
        tank.imuTurn(imu, TURN_SPEED / 3, 45, runtime, timeout);
        while (opModeIsActive() && (runtime.seconds() < timeout)) {
            telemetry.addData("Landing Adjustment", "Leg 1: %2.5f S Elapsed", runtime.seconds());
            telemetry.update();
        }
